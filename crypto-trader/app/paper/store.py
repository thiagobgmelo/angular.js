...
CREATE TABLE IF NOT EXISTS positions (
    ...
    events TEXT NOT NULL DEFAULT '[]'      -- histórico: tp1_hit, breakeven, stop...
);
...
class Store:
    ...
    def has_recent_signal(self, symbol: str, timeframe: str, direction: str, since_iso: str) -> bool:
    ...
    def positions(self, status: str | None = None) -> list[dict]:
        if status:
            ...
            rows = self.conn.execute("SELECT * FROM positions ORDER BY id DESC").fetchall()
        out = []
        for r in rows:
            d = dict(r)
            d["targets"] = json.loads(d["targets"])
    ...
    def update_position(self, pos_id: int, **fields) -> None:
        ...
        self.conn.execute(
            f"UPDATE positions SET {sets} WHERE id=?", (*fields.values(), pos_id)
        )
        self.conn.commit()
    ...
    def realized_pnl_total(self) -> float:
        row = self.conn.execute("SELECT COALESCE(SUM(realized_pnl),0) s FROM positions").fetchone()
        return float(row["s"])
